...
- **Symptom**: 95% accuracy even after fixing validation
- **Root Cause**: Contrastive loss negatives sampled randomly from OTHER trials. The network learned to match "acoustic fingerprints" — EEG and attended audio from the same trial share identical background noise, room acoustics, and recording artifacts. The network detected trial identity, bypassing attention entirely.
- **Fix**: Strict concurrent negative sampling — negative audio is ALWAYS the actual unattended track playing simultaneously in the opposite ear. Both tracks share identical recording conditions.
- **Impact**: Accuracy dropped from 95% to ~69%
- **Lesson**: The most insidious form of leakage in contrastive learning is systematic positive-negative asymmetry beyond the target variable.

### F-03: The 50% Accuracy Export Bug
- **Symptom**: Confidence export yielded exactly 50.09% — precisely chance
- **Root Cause**: Predictions evaluated against gender labels (1/2) instead of stream identity (A/B). Gender and stream identity are uncorrelated in the DTU experimental design.
- **Duration**: 3 days of debugging model weights, training pipelines, and audio preprocessing before the one-line root cause was found
- **Fix**: `correct = 1 if prediction == 'A' else 0`
- **Lesson**: Dataset label semantics are the #1 source of silent catastrophic failure.
...
### F-05: Temporal CNN LOSO Collapse
