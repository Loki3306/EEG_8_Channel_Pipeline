...
### D-02: Scipy IO Array Parsing Illusion
- **Discovery**: When using `scipy.io.loadmat` with `struct_as_record=False` on the AASD dataset, the `events` struct was parsed as an array of arrays (`numpy.ndarray`) rather than an object with attributes. Helper functions using `hasattr(ev, 'latency')` silently failed and returned the first element (the event type) for all fields. This created the illusion that the dataset's timing information was corrupted (i.e., `Type == Latency == Epoch`). 
- **Impact**: Removing abstractions and natively mapping the array indices (`[0]=type`, `[1]=latency`, `[4]=epoch`) fully restored the down-to-the-millisecond ground-truth dynamic attention labels.
- **Source**: Phase 28 Forensic Ground-Truth Verifier
- **Lesson**: Never use abstracted helper functions to parse unfamiliar or inconsistent MATLAB structs in Python. Always print the raw `type()` and data structures.

### D-03: 100% Stimulus Overlap
...
## Phase 41: Dataset-Wide Validation Disproves Universal Transfer
- **Finding**: Testing the Zero-Shot KUL -> AASD transfer on all 18 AASD subjects yielded a mean AUROC of **0.5028** (random guessing). S18 was a statistical anomaly (`0.5730`); for many subjects (e.g., S5, S11, S12), the zero-shot representation completely fails, scoring significantly below 0.50.
- **Finding**: Lightweight adapters (Latent or Projection) trained within-subject cannot rescue the broken representations. The improvement of Projection Adaptation over Zero-Shot is **not statistically significant** dataset-wide (`p = 0.2462`).
- **Implication**: The spatial and temporal filters learned on 64-channel KUL data do not generalize to 8-channel physical AASD data for most people. Transfer learning via a frozen backbone is a dead end. We must either deep-finetune or train from scratch natively on AASD.
