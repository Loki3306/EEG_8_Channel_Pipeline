...
**6. Phenotype-Specific Configurations (No Universal Hyperparameters):**
Do not force a universal frequency band (e.g., strictly <4Hz Syllabic) or a universal spatial filter across all subjects. Biological phenotype diversity (e.g., cortical folding, dipole orientations, skin impedance) means that different subjects respond optimally to different frequency bands (Broadband, Phonemic, Syllabic). When evaluating across multiple subjects, you must either sweep the bands per subject or use a subject-specific configuration mapping. Furthermore, account for Subject-Dependent Polarity Reversals (AUROC < 0.50) when analyzing results, as inverted dipoles are mathematically valid AAD signals.
