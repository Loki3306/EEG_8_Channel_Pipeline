...
def evaluate_windows(pred, ea, eb, window_samples):
    ...
    while start + window_samples <= len(pred):
        ...
        # Add tiny epsilon to variance to avoid division by zero
        if np.std(p) < 1e-12:
            # Constant prediction -> tie
            ca, cb = 0, 0
        else:
...
def main():
    print(f"\n===============================================================")
    print(f" STEP 1: SANITY VALIDATION ON 8-CHANNEL RIDGE PIPELINE")
    print(f"===============================================================")
    ...
    print(f"\n=> FINAL 8-CH NORMAL ACCURACY : {final_norm*100:.2f}%")
    print(f"=> FINAL 8-CH ZERO ACCURACY   : {final_zero*100:.2f}%")
    print(f"=> FINAL 8-CH SHUFFLE ACCURACY: {final_shuf*100:.2f}%\n")
