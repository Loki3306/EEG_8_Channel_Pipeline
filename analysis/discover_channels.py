...
def main():
    print("===============================================================")
    print(" DISCOVERING BEST EEG CHANNELS (64-CH RIDGE DECODER WEIGHTS)")
    print("===============================================================")
    ...
    # Accumulate the absolute weight of each channel across all training folds
    global_channel_importance = np.zeros(num_channels)
    num_folds = 0
    ...
    for held_out_path in paths:
        ...
        for ex in exs:
            ...
            # Only need target env for training the decoder
        ...
        for test_idx in range(len(preprocessed_trials)):
            ...
            for c in range(num_channels):
                # indices for channel c across all lags L
                idx = [c + L * num_channels for L in range(num_lags)]
                channel_importance[c] = np.sum(np.abs(final_weights[idx]))
                
            global_channel_importance += channel_importance
            num_folds += 1
            
        print(f"Processed subject {held_out_path.stem}...")
            
    global_channel_importance /= num_folds
    
    # Sort channels by importance
    ranked_channels = np.argsort(global_channel_importance)[::-1]
    
    print("\n===============================================================")
    print(" CHANNEL RANKING (Highest to Lowest Contribution)")
    print("===============================================================")
    for rank, c in enumerate(ranked_channels):
        print(f"Rank {rank+1:02d} | Channel {c:02d} | Score: {global_channel_importance[c]:.4f}")
