...
def main():
    print("\n" + "="*60)
    print("PHASE KUL-5: PARTIAL FINE-TUNING ON KUL (WHOLE EEG ENCODER)")
    print("="*60)
    ...
    print("Unfreezing entire EEG Encoder (Block 1 and Block 2)...")
    ...
    # Combine parameters for block 1 and block 2
    params_to_update = list(model.eeg_encoder.block1.parameters()) + list(model.eeg_encoder.block2.parameters())
    ...
    def evaluate_split(loader, trial_windows=None):
        ...
        return avg_loss, trial_acc, window_acc, mean_margin, mean_sim_a, mean_sim_b, mean_norm
    ...
    tr_loss, _, tr_wacc, tr_marg, tr_sa, tr_sb, tr_norm = evaluate_split(train_loader)
    te_loss, te_tacc, te_wacc, te_marg, te_sa, te_sb, te_norm = evaluate_split(test_loader, test_trial_windows)
    
    print(f"[Train] Loss: {tr_loss:.4f} | WinAcc: {tr_wacc*100:.1f}% | Margin: {tr_marg:.4f} | SimA: {tr_sa:.4f} | SimB: {tr_sb:.4f} | Norm: {tr_norm:.4f}")
    print(f"[Test]  Loss: {te_loss:.4f} | TrialAcc: {te_tacc*100:.1f}% | WinAcc: {te_wacc*100:.1f}% | Margin: {te_marg:.4f} | SimA: {te_sa:.4f} | SimB: {te_sb:.4f} | Norm: {te_norm:.4f}")
    ...
    save_path = os.path.join(checkpoint_dir, "matchnet_kul_finetuned_full_encoder.pth")
