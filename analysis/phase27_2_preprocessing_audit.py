...
def main():
    print("[INFO] Starting Phase 27.2 Preprocessing Mismatch Resolution Audit")
    ...
    out_dir = os.path.join(project_root, 'results', 'phase27_2')
    ...
    for sub in subjects:
        ...
        # --- FIX 1: 64-Channel Common Average Reference (CAR) ---
        # data_all is (64 channels, Time). Mean across axis 0 calculates the mean of all channels at each timepoint.
        ...
        for idx_ev, (ev_idx, audio_marker, trial_start_lat) in enumerate(trial_starts):
            ...
            # --- FIX 2: Z-score EEG over the entire trial ---
            ...
            # --- FIX 3: Z-score Audio over the entire trial ---
            ...
            for start in range(0, min(trial_eeg_8.shape[1], len(env_l_1d)) - win_len, hop):
                ...
                # NO PER-WINDOW STANDARDIZATION HERE
        ...
        for t_idx, (t_array, gt, m) in enumerate(trial_data):
            ...
            plt.title(f"{sub} - Trial {t_idx} (CAR + Trial Norm)")
        ...
        plt.title(f"{sub} - Margin Distributions (CAR + Trial Norm)")
        plt.legend()
