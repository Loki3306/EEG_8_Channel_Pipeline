...
class SubbandEEGNet(nn.Module):
    def __init__(self, in_channels=8, F1=8, D=2, F2=16, kernel_length=64, num_bands=28):
        super().__init__()
...
def train_eegnet_gammatone_screening(channels, lowcut, highcut):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} | Gammatone Subband EEGNet | Channels: {channels} | Band: {lowcut}-{highcut} Hz\n")
    ...
    for held_out_path, train_paths in folds:
        ...
        model = SubbandEEGNet(in_channels=len(channels), num_bands=NUM_BANDS).to(device)
    ...
    print("\n" + "="*50)
    print(f"\n[GAMMATONE SUBBAND EEGNET SCREENING RESULTS]")
    print("="*50)
    print(f" Normal EEG  : {final_normal*100:.2f}%")
    print("="*50)

def main():
    parser = argparse.ArgumentParser(description="Train Gammatone Subband EEGNet Screening")
    ...
    train_eegnet_gammatone_screening(args.channels, args.lowcut, args.highcut)
