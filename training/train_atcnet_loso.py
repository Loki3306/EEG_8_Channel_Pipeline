...
def train_atcnet_loso(lowcut, highcut):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} | Running ATCNet LOSO | Band: {lowcut}-{highcut} Hz")
    ...
    for held_out_path, train_paths in folds:
        ...
        model = ATCNet(in_channels=8).to(device)
    ...
    print("\n" + "="*50)
    print("ATCNET 8-CHANNEL LOSO RESULTS")
    print("="*50)
...
if __name__ == "__main__":
    ...
    train_atcnet_loso(args.lowcut, args.highcut)
