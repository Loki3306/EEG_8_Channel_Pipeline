...
def evaluate_ablation(model, eval_loader, device, ablation_type="None"):
    model.eval()
    ...
    with torch.no_grad():
        for b_e, b_a, b_y in eval_loader:
            ...
            if ablation_type == "Zero_EEG":
                b_e = torch.zeros_like(b_e)
            elif ablation_type == "Zero_Audio":
                b_a = torch.zeros_like(b_a)
            elif ablation_type == "Shuffled_Audio":
                # Shuffle the batch dimension to break the EEG-Audio temporal link
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 102: TRIAL-LEVEL SPLIT ABLATION SUITE")
    print(f" Testing S13 on Syllabic Band (< 4Hz)")
    print(f"=======================================================\n", flush=True)
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        auc = evaluate_ablation(model, eval_loader, device, "None")
        if auc > best_auc:
            best_auc = auc
            best_weights = {k: v.cpu() for k, v in model.state_dict().items()}
        print(f"  Epoch {epoch+1:02d}/{TRAIN_EPOCHS} - Val AUROC: {auc:.4f} (Best: {best_auc:.4f})")
            
    print("\n--- Training Complete. Running Ablations ---", flush=True)
    model.load_state_dict({k: v.to(device) for k, v in best_weights.items()})
    
    ablations = ["None", "Zero_EEG", "Zero_Audio", "Shuffled_Audio"]
    for ab in ablations:
        auc = evaluate_ablation(model, eval_loader, device, ab)
        print(f"[{ab:<15}] AUROC : {auc:.4f}")
