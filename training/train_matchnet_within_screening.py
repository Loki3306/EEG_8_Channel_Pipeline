...
def train_matchnet_within(eeg_model, channels, lowcut, highcut):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} | MatchNet Within-Subject ({eeg_model}) | Channels: {channels}")
    ...
    for subject_path in paths:
        sub_key = str(subject_path)
        print(f"\nEvaluating Within-Subject for: {subject_path.stem}")
        
        exs = subject_examples[sub_key]
        np.random.seed(42)
        np.random.shuffle(exs)
        
        # 5-fold CV (12 test, 48 train per fold)
        k_folds = 5
        ...
        for k in range(k_folds):
            ...
            train_exs_full = exs[:test_start] + exs[test_end:]
            
            val_split = int(0.1 * len(train_exs_full))
            val_exs = train_exs_full[:val_split]
            train_exs = train_exs_full[val_split:]
            
            # Use dummy prepare_dataset wrapper to reuse code
            X_tr_full, YA_tr_full, YB_tr_full = prepare_dataset(train_exs, channels, lowcut, highcut, subject_path.stem, mapping, envelopes)
            X_va_full, YA_va_full, YB_va_full = prepare_dataset(val_exs, channels, lowcut, highcut, subject_path.stem, mapping, envelopes)
            X_te_full, YA_te_full, YB_te_full = prepare_dataset(test_exs, channels, lowcut, highcut, subject_path.stem, mapping, envelopes)
            ...
            print(f"    Fold {k+1} Test Acc: {acc_norm*100:.2f}%")
            sub_accs.append(acc_norm)
            
        sub_mean = np.mean(sub_accs)
        print(f"  -> Subject Mean Acc : {sub_mean*100:.2f}%")
        all_accs_norm.append(sub_mean)
        
    final_acc_norm = np.mean(all_accs_norm)
    
    print("\n" + "="*50)
    print(f"[MATCHNET ({eeg_model.upper()}) WITHIN-SUBJECT SCREENING RESULTS]")
    print("="*50)
...
if __name__ == "__main__":
    ...
    train_matchnet_within(args.model, args.channels, args.lowcut, args.highcut)
