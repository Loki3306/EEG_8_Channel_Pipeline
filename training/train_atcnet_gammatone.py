...
class SubbandATCNet(nn.Module):
    def __init__(self, in_channels=8, F1=16, D=2, num_heads=2, tcn_depth=2, num_bands=28):
        super().__init__()
        # Use standard ATCNet, but change output projection
        self.base = ATCNet(in_channels=in_channels, F1=F1, D=D, num_heads=num_heads, tcn_depth=tcn_depth)
        # Override output projection
        hidden_dim = F1 * D
        self.base.output_proj = nn.Conv1d(hidden_dim, num_bands, kernel_size=1)
...
def evaluate_windows(pred, env_a, env_b, window_samples):
    ...
    while start + window_samples <= pred.shape[1]:
        ...
        if np.isnan(std_p) or std_p < 1e-12:
            ...
            # Flatten to calculate correlation over all 28 bands jointly
...
def evaluate_model(model, X, Y_A, Y_B, device, zero_eeg=False, shuffle_eeg=False, random_eeg=False):
    ...
    with torch.no_grad():
        for i in range(len(X)):
            ...
            pred = model(x).squeeze(0).cpu().numpy() # [28, Time]
...
def train_atcnet_gammatone_screening(channels, lowcut, highcut):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} | Gammatone Subband ATCNet | Channels: {channels} | Band: {lowcut}-{highcut} Hz\n")
    ...
    for held_out_path, train_paths in folds:
        ...
        model = SubbandATCNet(in_channels=len(channels), num_bands=NUM_BANDS).to(device)
        ...
        for epoch in range(100):
            ...
            for i in range(len(X_tr)):
                ...
                y = torch.FloatTensor(Y_tr[i]).unsqueeze(0).to(device) # shape: [1, 28, Time]
    ...
    print("\n" + "="*50)
    print(f"\n[GAMMATONE SUBBAND ATCNET SCREENING RESULTS]")
    print("="*50)
    print(f" Normal EEG  : {final_normal*100:.2f}%")
    print("="*50)

def main():
    parser = argparse.ArgumentParser(description="Train Gammatone Subband ATCNet Screening")
    ...
    train_atcnet_gammatone_screening(args.channels, args.lowcut, args.highcut)
