...
CALIB_LR = 1e-3       # Prevent tearing the FiLM parameters

EAR_CHANNEL_INDICES = [23, 31, 32, 40, 14, 22, 41, 49]

# -------------------------------------------------------------------------
# ARCHITECTURE (Phase 69: FiLM)
# -------------------------------------------------------------------------
class LocalEncoder(nn.Module):
    def __init__(self, in_channels, out_dim=64, use_film=False):
        super().__init__()
        ...
        if self.use_film:
            self.gamma1 = nn.Parameter(torch.ones(1, 16, 1))
            self.beta1 = nn.Parameter(torch.zeros(1, 16, 1))
            
        self.conv2 = nn.Conv1d(16, 32, kernel_size=17, padding=8)
        self.bn2 = nn.BatchNorm1d(32)
        
        if self.use_film:
            self.gamma2 = nn.Parameter(torch.ones(1, 32, 1))
            self.beta2 = nn.Parameter(torch.zeros(1, 32, 1))
...
class SequenceAADModel(nn.Module):
    def __init__(self, eeg_channels=8, latent_dim=64, lstm_hidden=64):
        super().__init__()
        # Replace Spatial Adapter with Deep FiLM Modulation in the EEG Encoder
        self.eeg_encoder = LocalEncoder(in_channels=eeg_channels, out_dim=latent_dim, use_film=True)
        self.aud_encoder = LocalEncoder(in_channels=1, out_dim=latent_dim, use_film=False)
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 69: FiLM CALIBRATION")
    print(" Upgraded Subject Adapter to Deep FiLM Modulation")
    print("=======================================================\n")
    ...
    for fold, (train_subjs, test_subjs) in enumerate(kf.split(subject_indices)):
        ...
        for epoch in range(PRETRAIN_EPOCHS):
            ...
            for b_e, b_a, b_b, b_y in train_loader:
                ...
                # FiLM L2 Regularization (Pull towards Identity)
                film_reg = 0.01 * (
                    ((model.eeg_encoder.gamma1 - 1.0)**2).sum() + 
                    (model.eeg_encoder.beta1**2).sum() +
                    ((model.eeg_encoder.gamma2 - 1.0)**2).sum() +
                    (model.eeg_encoder.beta2**2).sum()
                )
                total_loss = loss + film_reg
        ...
        print("\n--- STEP 2: TEST-TIME CALIBRATION (FiLM Only) ---")
        for subj in test_subjs:
            ...
            calib_params = [
                model.eeg_encoder.gamma1,
                model.eeg_encoder.beta1,
                model.eeg_encoder.gamma2,
                model.eeg_encoder.beta2
            ]
            ...
            for epoch in range(CALIB_EPOCHS):
                for b_e, b_a, b_b, b_y in calib_loader:
                    ...
                    # FiLM Regularization during calibration
                    film_reg = 0.001 * (
                        ((model.eeg_encoder.gamma1 - 1.0)**2).sum() + 
                        (model.eeg_encoder.beta1**2).sum() +
                        ((model.eeg_encoder.gamma2 - 1.0)**2).sum() +
