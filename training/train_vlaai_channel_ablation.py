...
CHANNEL_SUBSETS = {
    "2-Channel": [47, 30],
    "4-Channel": [47, 30, 12, 49],
    "8-Channel": [47, 12, 49, 31, 17, 53, 30, 37]
}
...
def prepare_dataset(examples, channels):
    X = []
    Y = []
    Y_A = []
    Y_B = []
...
def train_loso_ablation(name, channels):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n===============================================================")
    print(f" RUNNING VLAAI-LITE ABLATION: {name} ({len(channels)} channels)")
    print(f"===============================================================")
    ...
    for held_out_path, train_paths in folds:
        ...
        X_tr, Y_tr, YA_tr, YB_tr = prepare_dataset(train_exs, channels)
        X_va, Y_va, YA_va, YB_va = prepare_dataset(val_exs, channels)
        X_te, Y_te, YA_te, YB_te = prepare_dataset(test_exs, channels)
        
        model = VLAAILite(in_channels=len(channels)).to(device)
    ...
    print("\n" + "="*50)
    print(f"VLAAI-LITE {name} LOSO RESULTS")
    print("="*50)
...
def main():
    for name, channels in CHANNEL_SUBSETS.items():
        train_loso_ablation(name, channels)
