...
def process_trial_unsupervised(tr, device):
    # Extract raw data
    ...
    for fold in range(N_FOLDS):
        ...
        # Train Ridge on the other 45 seconds
        ...
        # Test on the held-out 15 seconds
        Y_hat_L = (X_test @ W_L).squeeze(-1)
        Y_hat_R = (X_test @ W_R).squeeze(-1)
        
        r_L = batch_pearsonr_pt(Y_hat_L.unsqueeze(0), YL_test.unsqueeze(0)).item()
        r_R = batch_pearsonr_pt(Y_hat_R.unsqueeze(0), YR_test.unsqueeze(0)).item()
        
        scores_L.append(r_L)
        scores_R.append(r_R)
        
    final_score_L = np.mean(scores_L)
    final_score_R = np.mean(scores_R)
    
    # Target Meta (For AUROC evaluation)
    sp = tr['meta']['switch_points']
    # The label for the trial is essentially the dominant speaker (or we can just evaluate AUROC for predicting L)
    # The AASD dataset block design means the attended speaker switches, but usually it's constant for the whole trial, OR
    # we just take the first label since we are aggregating the whole trial into one decision.
    current_spk = sp[0][0] if len(sp) > 0 else 'L'
    Y_meta = 1.0 if current_spk == 'L' else 0.0
    
    return final_score_L, final_score_R, Y_meta
...
def process_subject(cache_file, device_id):
    ...
    for t_idx in range(N_TRIALS):
        # We treat EVERY trial as a completely independent test trial
        # No cross-trial static model is trained!
        score_L, score_R, y_meta = process_trial_unsupervised(cached[t_idx], device)
    ...
    print(f"  [{subj_name}] Finished on {device}. Unsupervised Adaptive AUROC: {global_auc:.4f}")
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 129: UNSUPERVISED ADAPTIVE DECODING (WITHIN-TRIAL CCA)")
    ...
    print("\n=======================================================")
    print(" PHASE 129 UNSUPERVISED ADAPTIVE RESULTS")
    print("=======================================================")
