...
def process_subject(cache_file, device_id):
    ...
    for test_idx in range(N_TRIALS):
        ...
        # 2. Continuous 5-second decoding (SLIDING 1-SECOND STEP)
        for start in range(0, T_eff - WIN_SAMPLES + 1, STEP_SAMPLES):
        ...
        if len(window_diffs) > 0:
            total_valid_windows += len(window_diffs)
    ...
    print(f"  [{subj_name}] P-MoE Integrated AUROC: {global_auc:.4f} (Valid Windows: {total_valid_windows}) | Favored: S({expert_track['Static']}) M({expert_track['MAP']}) A({expert_track['ABS']})")
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 139: SLIDING BLACKOUT REINTEGRATION")
    ...
    print("\n=======================================================")
    print(" PHASE 139 REINTEGRATED MoE RESULTS")
    print("=======================================================")
