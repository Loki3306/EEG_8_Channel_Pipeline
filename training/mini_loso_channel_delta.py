...
def run_study():
    ...
    variants = ["Baseline", "SetB", "DeltaEEG", "DeltaEEG+SetB"]
    ...
    for held_out_subj in TARGET_SUBJECTS:
        ...
        # 2. Set B Only
        print(f"\n[ Variant 2: SetB Only ]")
        data = prepare_data_for_variant(subject_examples, train_paths, held_out_path, SET_B_CHANNELS, mapping, envelopes, use_delta=False)
        acc_2s, acc_5s = train_and_eval(len(SET_B_CHANNELS), *data, device)
        results[held_out_subj]["SetB"]["2s"] = acc_2s
        results[held_out_subj]["SetB"]["5s"] = acc_5s
    ...
    print("="*80)
    print("| Subject | Baseline | SetB     | DeltaEEG | DeltaEEG+SetB |")
    print("| ------- | -------- | -------- | -------- | ------------- |")
    for subj in TARGET_SUBJECTS:
        if subj not in results or "Baseline" not in results[subj] or "2s" not in results[subj]["Baseline"]: continue
        r = results[subj]
        print(f"| {subj:7s} | {r['Baseline']['2s']*100:8.2f} | {r['SetB']['2s']*100:8.2f} | {r['DeltaEEG']['2s']*100:8.2f} | {r['DeltaEEG+SetB']['2s']*100:13.2f} |")
    ...
    for subj in TARGET_SUBJECTS:
        ...
        r = results[subj]
        print(f"| {subj:7s} | {r['Baseline']['5s']*100:8.2f} | {r['SetB']['5s']*100:8.2f} | {r['DeltaEEG']['5s']*100:8.2f} | {r['DeltaEEG+SetB']['5s']*100:13.2f} |")
    ...
    for v in ["SetB", "DeltaEEG", "DeltaEEG+SetB"]:
