...
def get_attended_speaker_at_time(start_idx, switch_points):
    """
    Returns 'L' or 'R' representing the attended speaker at start_idx.
    """
    # If no switches or before first switch, the attended speaker is determined by the first cue.
    # Wait: The cue at t=0 is recorded as a switch point at latency 0.
    current_spk = 'L' # Default fallback
...
def extract_windows_for_trial(trial):
    """
    Slices a 60s trial into 2.0s windows, filtering out cognitive gaps.
    Returns: X (windows, channels, time), y (windows,)
    """
    ...
    for start in range(0, T - WIN_SAMPLES + 1, WIN_SAMPLES):
        ...
        # Determine label (1 for Left, 0 for Right)
...
def main():
    ...
    # We will do 5-Fold LOTO CV (Evaluate on held-out trials)
    kf = KFold(n_splits=5, shuffle=False)
    
    fold_aurocs = []
    
    print("\n=======================================================")
    print(" PHASE 49: NATIVE EEGNET DIRECT CLASSIFICATION ON S1")
    print("=======================================================\n")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(trials)):
        ...
        for epoch in range(EPOCHS):
            ...
            try:
                ...
                val_auc = 0.50 # If only one class is present in a tiny batch
