...
# The 8 electrodes physically surrounding the ears
...
class LocalEncoder(nn.Module):
    def __init__(self, in_channels, out_dim=64, is_eeg=False):
        ...
        if is_eeg:
            # 1. Spatial Unmixing (Data-Driven CSP)
            self.spatial_mixer = nn.Conv1d(in_channels, in_channels, kernel_size=1)
            
        # 2. Dilated Temporal Convolutions
        self.conv1 = nn.Conv1d(in_channels, 16, kernel_size=17, padding=16, dilation=2)
        self.gn1 = nn.GroupNorm(4, 16) # GroupNorm is more stable for small batches
        
        self.conv2 = nn.Conv1d(16, 32, kernel_size=9, padding=16, dilation=4)
...
class SequenceAADModel(nn.Module):
    ...
    def forward(self, eeg_seq, aud_a_seq, aud_b_seq, labels_seq=None, hidden=None):
        ...
        sim_a = F.cosine_similarity(p_eeg, p_a, dim=-1)
        sim_b = F.cosine_similarity(p_eeg, p_b, dim=-1)
        
        contrastive_loss = torch.tensor(0.0, device=eeg_seq.device)
        
        if labels_seq is not None:
            # 3. Binary Contrastive Margin Loss
            logit_scale = self.logit_scale.exp().clamp(max=100)
            logits_a = sim_a * logit_scale
            logits_b = sim_b * logit_scale
            
            contrastive_logits = torch.stack([logits_a, logits_b], dim=1) # (B*SeqLen, 2)
            
            # labels_seq: 1 = Attended A, 0 = Attended B
            # So if 1, target is 0. If 0, target is 1.
            target_indices = (1 - labels_seq.view(B * SeqLen)).long()
            contrastive_loss = F.cross_entropy(contrastive_logits, target_indices)
        
        score_diff = sim_a - sim_b
        lstm_feat = torch.cat([
            p_eeg, p_a, p_b, 
            sim_a.unsqueeze(-1), sim_b.unsqueeze(-1), score_diff.unsqueeze(-1)
        ], dim=-1).reshape(B, SeqLen, -1)
        
        lstm_out, hidden = self.lstm(lstm_feat, hidden)
        clf_logits = self.classifier(lstm_out).squeeze(-1)
        
        return clf_logits, contrastive_loss
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 60: ULTIMATE 8-CHANNEL OPTIMIZED ARCHITECTURE")
    print("=======================================================\n")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(all_trials)):
        ...
        train_loader = DataLoader(AASDSequenceDataset(train_trials), batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True)
        test_loader = DataLoader(AASDSequenceDataset(test_trials), batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True)
        ...
        for epoch in range(EPOCHS):
            ...
            for b_e, b_a, b_b, b_y in train_loader:
                ...
                logits, contrastive_loss = model(b_e, b_a, b_b, b_y)
                bce_loss = criterion_bce(logits, b_y)
                
                # Total loss applies a 0.2 weight to the auxiliary contrastive objective
    ...
    print(f"\nAVERAGE 5-FOLD PHASE 60 AUROC: {np.mean(fold_aurocs):.4f}")
