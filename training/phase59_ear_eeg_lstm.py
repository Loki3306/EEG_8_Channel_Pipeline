...
BATCH_SIZE = 256
EPOCHS = 10 # Dropped from 20 since Phase 58 proved it converges by Epoch 2
LR = 1e-3

# The 8 electrodes physically surrounding the ears (Temporal & Parietal lobes)
# Extracted from the AASD Neuroscan 64-channel montage:
# T7(23), T8(31), TP7(32), TP8(40), FT7(14), FT8(22), P7(41), P8(49)
...
class AASDSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        eeg_seq = tr['eeg'][:, seq_start:seq_start + SEQ_SAMPLES] # (8, 1280)
...
def main():
    ...
    if len(cache_files) == 0:
        print("No cache files found. Please run generate_aasd_cache.py first.")
        return
        
    print("\n=======================================================")
    print(" PHASE 59: EAR-EEG 8-CHANNEL CALIBRATED MODEL (LOTO)")
    print("=======================================================\n")
    ...
    print(f"EEG Tensor Shape per trial: {all_trials[0]['eeg'].shape} (8 Channels)")
    ...
    print(f"\nAVERAGE 5-FOLD 8-CHANNEL EAR-EEG AUROC: {np.mean(fold_aurocs):.4f}")
