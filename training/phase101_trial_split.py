...
from models.aad_cosine import PureCosineAADModel
...
# The 4 most distinct biological phenotypes from Phase 96
TEST_BANDS = [
    ("Broadband", 0.5, 32.0),
    ("Syllabic (LPF < 4Hz)", None, 4.0),
    ("Phonemic (4-8Hz)", 4.0, 8.0),
    ("Transients (HPF > 16Hz)", 16.0, None)
]
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 101: TRIAL-LEVEL SPLIT BASELINE")
    print(f" Zero overlap leakage, Zero chronological leakage")
    print(f"=======================================================\n", flush=True)
    ...
    for subject in TEST_SUBJECTS:
        ...
        for band_name, lowcut, highcut in TEST_BANDS:
            print(f"\n  --- Band: {band_name} ---", flush=True)
            ...
            # Shuffle sequences within the sets for stochastic gradient descent
            ...
            model = PureCosineAADModel(encoder_type='baseline', audio_channels=16).to(device)
            ...
            for epoch in range(TRAIN_EPOCHS):
                ...
                if len(np.unique(all_labels)) > 1:
                    ...
                    if epoch == TRAIN_EPOCHS - 1: # Last epoch diagnostics
                        labels_arr = np.array(all_labels)
                        logits_arr = np.array(all_logits)
                        mean_match_logit = np.mean(logits_arr[labels_arr == 1.0])
                        mean_mismatch_logit = np.mean(logits_arr[labels_arr == 0.0])
                        print(f"    -> [Diagnostic] Mean Match Logit: {mean_match_logit:.4f}, Mean Mismatch Logit: {mean_mismatch_logit:.4f}")
