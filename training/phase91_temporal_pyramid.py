...
# Evaluate all 33 subjects to observe the cohort-wide response to the Temporal Pyramid
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 91: MULTI-RESOLUTION TEMPORAL PYRAMID")
    print(" Physically downsampling audio to separate transient and semantic features")
    print(" (No Gating Networks. No WavLM. Pure Multiband Additive Fusion)")
    print("=======================================================\n", flush=True)
    ...
    for subj_idx in EVAL_SUBJECTS:
        ...
        # num_workers=0 since data is in RAM, avoids IPC bottleneck
        ...
        # Instantiate model with use_pyramid=True
        model = TCNAADModel(audio_channels=16, use_pyramid=True).to(device)
        ...
        for epoch in range(TRAIN_EPOCHS):
            ...
            if len(np.unique(all_labels)) > 1:
                ...
                # Tracking MAX AUC to measure representation capacity
                best_auc = max(best_auc, auc)
                
            print(f"  Epoch {epoch+1}/15 - AUC: {auc:.4f}", flush=True)
                
        print(f"  -> FINAL AUROC (MAX): {best_auc:.4f}\n", flush=True)
