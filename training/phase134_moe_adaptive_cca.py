...
def calc_stability(W_list):
    if len(W_list) < 2:
        return 1.0
    sims = []
    for i in range(len(W_list)):
        for j in range(i+1, len(W_list)):
            cos = torch.nn.functional.cosine_similarity(W_list[i], W_list[j], dim=0)
            sims.append(cos.item())
    return max(0.0, np.mean(sims))
...
def evaluate_expert_for_stream(X_test_pca, Y_test, W_prior, expert_type, device):
    ...
    if expert_type == 'Static':
        ...
        stability = calc_stability(W_folds)
        
    return mean_z, stability
...
def process_subject(cache_file, device_id):
    ...
    for test_idx in range(N_TRIALS):
        ...
        for Y_cand in [YL_test, YR_test]:
            z_S, S_S = evaluate_expert_for_stream(X_test_pca, Y_cand, W_prior, 'Static', device)
            z_M, S_M = evaluate_expert_for_stream(X_test_pca, Y_cand, W_prior, 'MAP', device)
            z_A, S_A = evaluate_expert_for_stream(X_test_pca, Y_cand, W_prior, 'ABS', device)
            
            # Evidence computation (harsh stability penalty)
            E_S = (S_S ** 3) * abs(z_S)
            E_M = (S_M ** 3) * abs(z_M)
            E_A = (S_A ** 3) * abs(z_A)
            ...
            z_final = weights[0]*z_S + weights[1]*z_M + weights[2]*z_A
        ...
        # Unsigned Decision Rule for ultimate invariance!
        z_L, z_R = final_zs
    ...
    print(f"  [{subj_name}] MoE AUROC: {global_auc:.4f} | Favored Experts: Static({expert_track['Static']}) MAP({expert_track['MAP']}) ABS({expert_track['ABS']})")
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 134: MIXTURE OF EXPERTS ADAPTIVE CCA")
    ...
    print("\n=======================================================")
    print(" PHASE 134 MIXTURE OF EXPERTS RESULTS")
    print("=======================================================")
    print(f"{'Subject':<10} {'MoE AUROC':<10}")
