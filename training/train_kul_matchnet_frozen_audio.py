    ...
    with torch.no_grad():
        for t in test_data:
            ...
            if trial_sim_a:
                ...
                print(f"    Trial {meta.get('TrialID', 0):02d} | Exp: {meta.get('experiment', 'Unknown')} | Track Attended: {meta.get('attended_track', 'Unknown')} | Pred: {pred} | Margin: {margin:.4f}")
...
def train_matchnet_kul_loso(dataset_dir=None, eeg_model_type="eegnet", epochs=50, batch_size=32, lr=1e-3, target_fold=None, dtu_ckpt=None):
    ...
    for held_out_idx, held_out_id in enumerate(sorted(subject_paths)):
        ...
        # Initialize model
        model = ContrastiveMatchNet(eeg_model_type, eeg_channels=8, audio_channels=28, latent_dim=64).to(device)
