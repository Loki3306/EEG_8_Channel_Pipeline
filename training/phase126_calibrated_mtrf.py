...
def process_subject(cache_file, device_id, male_assignments):
    ...
    for test_idx in range(N_TRIALS):
        ...
        for tr_idx in train_indices:
            tr_info = data_cache[tr_idx]
            score_L, score_R = evaluate_trial_scores(W_final_L, W_final_R, tr_info)
            if score_L is not None:
                male_ch = male_assignments[tr_idx + 1] # 1-indexed
                score_L = score_L.cpu().numpy()
                score_R = score_R.cpu().numpy()
                if male_ch == 'L':
                    scores_LM.extend(score_L)
                    scores_RF.extend(score_R)
                else:
                    scores_LF.extend(score_L)
                    scores_RM.extend(score_R)
                    
        # Compute Calibration Statistics
        mu_LM, std_LM = np.mean(scores_LM), np.std(scores_LM) + 1e-8
        mu_LF, std_LF = np.mean(scores_LF), np.std(scores_LF) + 1e-8
        mu_RM, std_RM = np.mean(scores_RM), np.std(scores_RM) + 1e-8
        mu_RF, std_RF = np.mean(scores_RF), np.std(scores_RF) + 1e-8
        ...
        if score_L is not None:
            score_L = score_L.cpu().numpy()
            score_R = score_R.cpu().numpy()
            male_ch = male_assignments[test_idx + 1]
            
            # Z-Score Calibration
            if male_ch == 'L':
                z_L = (score_L - mu_LM) / std_LM
                z_R = (score_R - mu_RF) / std_RF
            else:
                z_L = (score_L - mu_LF) / std_LF
                z_R = (score_R - mu_RM) / std_RM
    ...
    print(f"  [{subj_name}] Finished on {device}. Calibrated AUROC: {global_auc:.4f}")
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 126: GPT CALIBRATED mTRF")
    ...
    print("\n=======================================================")
    print(" PHASE 126 CALIBRATED mTRF RESULTS")
    print("=======================================================")
