...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 76: FROM-SCRATCH SCALING LAWS")
    print(" Training completely random networks on varying data fractions")
    print(" Hypothesis: Avoids the anti-correlation trap of Universal Pre-Training")
    print("=======================================================\n")
    ...
    for subj in range(len(all_subj_seqs)):
        print(f"\n==================== SUBJECT {subj:02d} ====================")
        ...
        for frac in DATA_FRACTIONS:
            ...
            for epoch in range(TRAIN_EPOCHS):
                ...
                current_auc = evaluate(model)
                best_auc = max(best_auc, current_auc)
                model.train()
                    
            results[frac].append(best_auc)
            
            calib_time_sec = 3.5 + (num_calib - 1) * 0.5 if num_calib > 0 else 0
            calib_time_min = calib_time_sec / 60.0
            
            print(f"  - Train {frac*100:5.1f}% ({num_calib:4d} seqs | ~{calib_time_min:4.1f} min): {best_auc:.4f}")

    print("\n=======================================================")
    print(f" FINAL FROM-SCRATCH SCALING LAWS (Mean across 18 Subjects):")
    for frac in DATA_FRACTIONS:
        if len(results[frac]) > 0:
            print(f" Train {frac*100:5.1f}%:   {np.mean(results[frac]):.4f}")
    print("=======================================================\n")
