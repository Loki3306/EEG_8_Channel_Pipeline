...
# The 3 Information-Starved subjects from Phase 79/80 (Plus Subject 17 as a control)
...
def extract_multiband_sequences(trials):
    sequences = []
    seq_hop = int(0.5 * SR) 
    
    for tr in trials:
        eeg = tr['eeg']
        env_l = tr['env_l'].numpy() # [16, Time_EEG]
        ...
        for i in range(len(boundaries) - 1):
            ...
            if safe_end - safe_start >= SEQ_SAMPLES:
                for seq_start in range(safe_start, safe_end - SEQ_SAMPLES + 1, seq_hop):
                    ...
                    a_seq = env_l[:, seq_start:seq_start + SEQ_SAMPLES] # [16, SEQ_SAMPLES]
                    b_seq = env_r[:, seq_start:seq_start + SEQ_SAMPLES]
                    
                    e = e_seq.unfold(-1, WIN_SAMPLES, HOP_SAMPLES).permute(1, 0, 2) # [num_windows, 60, WIN_SAMPLES]
                    
                    a_tensor = torch.from_numpy(a_seq) # [16, SEQ_SAMPLES]
                    a_windows = a_tensor.unfold(-1, WIN_SAMPLES, HOP_SAMPLES).permute(1, 0, 2) # [num_windows, 16, WIN_SAMPLES]
                    
                    b_tensor = torch.from_numpy(b_seq)
                    ...
                    sequences.append((e, a_windows, b_windows, y))
    return sequences
...
def main():
    ...
    if len(cache_files) == 0:
        print("No cache files found. (Run on Kaggle after generate_multiband_cache.py)")
        return
        
    print("\n=======================================================")
    print(" PHASE 81: MULTIBAND ARCHITECTURE EVALUATION")
    print(" Replacing WavLM with 16-Band Cochlear Envelopes.")
    ...
    for subj_idx in EVAL_SUBJECTS:
        ...
        seqs = extract_multiband_sequences(subj_trials)
        ...
        auc_tcn = train_and_eval(TCNAADModel, "Multiband + Dilated TCN", use_wavlm=False, audio_channels=16, dropout=0.3)
