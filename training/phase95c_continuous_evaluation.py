...
from models.aad_continuous import ContinuousAADAUDModel
...
class ContinuousAASDDataset(Dataset):
...
def train_and_eval_continuous(subj_idx, train_loader, eval_loader, device):
    model = ContinuousAADAUDModel().to(device)
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 95c: SYMMETRIC CONTINUOUS SEQUENCE (BI-LSTM)")
    print(" Architecture: Symmetric Siamese + Dilated Frontend (445ms lag compensation)")
    print(" Speed: Replaces 3,136 Python operations per batch with native cuDNN.")
    ...
    for subj_idx in EVAL_TARGETS:
        ...
        train_loader = DataLoader(ContinuousAASDDataset(calib_pool), batch_size=BATCH_SIZE, shuffle=True, pin_memory=True, num_workers=0)
        eval_loader = DataLoader(ContinuousAASDDataset(eval_set), batch_size=BATCH_SIZE, shuffle=False, pin_memory=True, num_workers=0)
        
        auc = train_and_eval_continuous(subj_idx, train_loader, eval_loader, device)
        
        print(f"  Continuous LSTM AUROC : {auc:.4f}")
