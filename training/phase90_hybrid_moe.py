...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 90: HYBRID MIXTURE-OF-EXPERTS (MoE)")
    print(" Dynamically routing between Semantic (WavLM) and Cochlear (16-Band).")
    print("=======================================================\n", flush=True)
    ...
    for subj_idx in EVAL_SUBJECTS:
        ...
        for epoch in range(TRAIN_EPOCHS):
            ...
            avg_alpha = total_alpha / max(1, steps)
            ...
            with torch.no_grad():
                for b_e, b_wa, b_wb, b_ma, b_mb, b_y in eval_loader:
                    ...
                    eval_alphas.append(alpha.mean().item())
            
            if len(np.unique(all_labels)) > 1:
                auc = roc_auc_score(all_labels, all_preds)
                best_auc = max(best_auc, auc)
                
            print(f"  Epoch {epoch+1}/15 - AUC: {auc:.4f} | Avg Alpha (WavLM weight): {np.mean(eval_alphas):.4f}", flush=True)
