...
def train_and_eval(attention_type, train_loader, eval_loader, device):
    model = TCNAADModel(
        eeg_channels=8, 
        latent_dim=64, 
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 87: MULTIBAND COHORT ATTENTION EVALUATION")
    print(" Evaluating Baseline, Spatial-Spectral, and Cross-Modal architectures")
    print(" across ALL available subjects to observe overall dataset growth.")
    print("=======================================================\n", flush=True)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Training on Device: {device}\n", flush=True)
    
    architectures = ['none', 'spatial_spectral', 'cross_modal']
    results = {arch: [] for arch in architectures}
    
    for subj_idx, cache_file in enumerate(cache_files):
        print(f"\n--- Subject {subj_idx+1:02d} ---", flush=True)
        
        try:
            cached_data = torch.load(cache_file, map_location='cpu', weights_only=False)['raw']
            ...
            if len(eval_set) == 0 or len(calib_pool) == 0:
                print(f"  Skipping Subject {subj_idx+1:02d}: Not enough data")
                continue
            ...
            for arch in architectures:
                auc = train_and_eval(arch, train_loader, eval_loader, device)
                results[arch].append(auc)
                subj_results[arch] = auc
                
            print(f"  Baseline TCN     : {subj_results['none']:.4f}")
            print(f"  Spatial-Spectral : {subj_results['spatial_spectral']:.4f}")
            print(f"  Cross-Modal Gate : {subj_results['cross_modal']:.4f}")
            
        except Exception as e:
            print(f"  Failed Subject {subj_idx+1:02d}: {e}")
            
    print("\n=======================================================")
    print(" FINAL COHORT AVERAGES")
    print("=======================================================")
    for arch in architectures:
        if len(results[arch]) > 0:
            avg_auc = np.mean(results[arch])
            print(f"  {arch.ljust(18)} : {avg_auc:.4f} (n={len(results[arch])})")
