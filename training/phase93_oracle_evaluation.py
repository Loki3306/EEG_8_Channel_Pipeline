...
def train_and_eval(model, train_loader, eval_loader, device):
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 93: ORACLE TEMPORAL RESOLUTION EVALUATION")
    print(" Training purely Fast (128Hz) and purely Slow (32Hz) models")
    print(" Oracle = max(Fast, Slow). This establishes the Absolute Upper Bound!")
    print("=======================================================\n", flush=True)
    ...
    for subj_idx in EVAL_SUBJECTS:
        ...
        # Train Fast Model
        fast_model = TCNAADModel(audio_channels=16, encoder_type='fast').to(device)
        fast_auc = train_and_eval(fast_model, train_loader, eval_loader, device)
        
        # Train Slow Model
        slow_model = TCNAADModel(audio_channels=16, encoder_type='slow').to(device)
        slow_auc = train_and_eval(slow_model, train_loader, eval_loader, device)
        
        oracle_auc = max(fast_auc, slow_auc)
        optimal_encoder = "FAST" if fast_auc > slow_auc else "SLOW"
        
        print(f"  Fast (128Hz) AUROC : {fast_auc:.4f}", flush=True)
        print(f"  Slow (32Hz)  AUROC : {slow_auc:.4f}", flush=True)
        print(f"  -> ORACLE AUROC    : {oracle_auc:.4f} (Optimal: {optimal_encoder})\n", flush=True)
