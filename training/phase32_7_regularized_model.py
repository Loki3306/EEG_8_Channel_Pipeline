...
class ShallowAudioEncoder(nn.Module):
    def __init__(self, F2=16):
        super().__init__()
        # Single convolutional layer to extract simple envelope features without memorizing entire audio tracks
        self.cnn = nn.Sequential(
            nn.Conv1d(1, F2, kernel_size=33, padding=16, bias=False),
            nn.BatchNorm1d(F2),
            nn.GELU(),
            nn.Dropout(0.7) # Extreme dropout to prevent memorization
        )
...
class ShallowContrastiveMatchNet(nn.Module):
    def __init__(self, eeg_channels=64):
        super().__init__()
        self.eeg_enc = EEGEncoder(in_channels=eeg_channels)
        self.aud_enc = ShallowAudioEncoder()
...
def run_pairwise_softmax_no_norm():
    ...
    model = ShallowContrastiveMatchNet(eeg_channels=num_eeg_channels).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-2) # High weight decay
