...
def batch_fast_ridge_pt(XTX_batch, XTy_batch, lam=100.0):
    ...
    # Adding jitter to diagonal for absolute stability
    jitter = 1e-6 * torch.randn(B, F, F, device=XTX_batch.device, dtype=XTX_batch.dtype) * I
    return torch.linalg.solve(XTX_batch + lam * I + jitter, XTy_batch)
...
def prepare_trial_data(tr, band_idx, device):
    ...
    XTX_L = (X_L.T @ X_L).unsqueeze(0) if X_L.shape[0] > 0 else torch.zeros((1, F, F), device=device)
    XTy_L = (X_L.T @ Y_L).unsqueeze(0).unsqueeze(-1) if X_L.shape[0] > 0 else torch.zeros((1, F, 1), device=device)
    
    XTX_R = (X_R.T @ X_R).unsqueeze(0) if X_R.shape[0] > 0 else torch.zeros((1, F, F), device=device)
    XTy_R = (X_R.T @ Y_R).unsqueeze(0).unsqueeze(-1) if X_R.shape[0] > 0 else torch.zeros((1, F, 1), device=device)
...
def evaluate_trial_directional(W_L, W_R, tr_info):
    if len(tr_info['seq_indices']) == 0:
        return torch.tensor([], device=W_L.device), torch.tensor([], device=W_L.device)
...
def process_subject(cache_file, device_id):
    ...
    for test_idx in range(N_TRIALS):
        ...
        for b_idx in range(len(BANDS)):
            XTX_train_full_L = sum([data_cache[b_idx][i]['XTX_L'] for i in train_indices])
            XTy_train_full_L = sum([data_cache[b_idx][i]['XTy_L'] for i in train_indices])
            
            XTX_train_full_R = sum([data_cache[b_idx][i]['XTX_R'] for i in train_indices])
            XTy_train_full_R = sum([data_cache[b_idx][i]['XTy_R'] for i in train_indices])
            
            # Vectorized Inner LOTO for L and R independently
            XTX_subtrain_L = torch.cat([XTX_train_full_L - data_cache[b_idx][v]['XTX_L'] for v in train_indices], dim=0)
            XTy_subtrain_L = torch.cat([XTy_train_full_L - data_cache[b_idx][v]['XTy_L'] for v in train_indices], dim=0)
            
            XTX_subtrain_R = torch.cat([XTX_train_full_R - data_cache[b_idx][v]['XTX_R'] for v in train_indices], dim=0)
            XTy_subtrain_R = torch.cat([XTy_train_full_R - data_cache[b_idx][v]['XTy_R'] for v in train_indices], dim=0)
            
            W_L_batch = batch_fast_ridge_pt(XTX_subtrain_L, XTy_subtrain_L, lam=100.0)
            W_R_batch = batch_fast_ridge_pt(XTX_subtrain_R, XTy_subtrain_R, lam=100.0)
            
            for batch_i, val_idx in enumerate(train_indices):
                diffs, labels = evaluate_trial_directional(W_L_batch[batch_i], W_R_batch[batch_i], data_cache[b_idx][val_idx])
                if len(diffs) > 0:
        ...
        for b_idx in top_2_b_idx:
            XTX_train_full_L = sum([data_cache[b_idx][i]['XTX_L'] for i in train_indices])
            XTy_train_full_L = sum([data_cache[b_idx][i]['XTy_L'] for i in train_indices])
            W_final_L = batch_fast_ridge_pt(XTX_train_full_L, XTy_train_full_L, lam=100.0)[0]
            
            XTX_train_full_R = sum([data_cache[b_idx][i]['XTX_R'] for i in train_indices])
            XTy_train_full_R = sum([data_cache[b_idx][i]['XTy_R'] for i in train_indices])
            W_final_R = batch_fast_ridge_pt(XTX_train_full_R, XTy_train_full_R, lam=100.0)[0]
            
            diffs, labels = evaluate_trial_directional(W_final_L, W_final_R, data_cache[b_idx][test_idx])
    ...
    print(f"  [{subj_name}] Finished on {device}. Directional AUROC: {global_auc:.4f}")
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 116: DIRECTIONAL LOTO (SOLVING SPATIAL INVERSION)")
    ...
    print("\n=======================================================")
    print(" PHASE 116 DIRECTIONAL LOTO CLINICAL CALIBRATION RESULTS")
    print("=======================================================")
