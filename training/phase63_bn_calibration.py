...
BATCH_SIZE = 256
PRETRAIN_EPOCHS = 10
CALIB_EPOCHS = 50 # Increased from 10 to 50 to allow enough gradient steps
LR = 1e-3

EAR_CHANNEL_INDICES = [23, 31, 32, 40, 14, 22, 41, 49]

# -------------------------------------------------------------------------
# ARCHITECTURE (Phase 63: BN Calibration Adapter)
# -------------------------------------------------------------------------
...
def main():
    ...
    print("\n=======================================================")
    print(" PHASE 63: TRUE CALIBRATION WITH BN UNLOCK")
    print("=======================================================\n")
    ...
    train_subjs = list(range(14))
    test_subjs = list(range(14, 18))
    
    universal_trials = []
    for i in train_subjs:
        universal_trials.extend(all_trials_by_subj[i])
        
    print(f"\n--- STEP 1: PRE-TRAINING UNIVERSAL BACKBONE on {len(train_subjs)} Subjects ---")
    ...
    for epoch in range(PRETRAIN_EPOCHS):
        ...
        for b_e, b_a, b_b, b_y in train_loader:
            ...
            # Identity Regularization to keep the Adapter as a pass-through during pre-training
    ...
    print("\nPre-training Complete! Saving Universal Weights...")
    universal_weights = copy.deepcopy(model.state_dict())
    
    print(f"\n--- STEP 2: TEST-TIME CALIBRATION on {len(test_subjs)} Unseen Subjects ---")
    calibrated_aurocs = []
    zeroshot_aurocs = []
    
    for subj in test_subjs:
        print(f"\nEvaluating Subject {subj+1} (Totally Unseen)")
        subj_trials = all_trials_by_subj[subj]
        
        # 10 trials is ~1 minute of Audio AAD. The rest is testing.
        num_calib_trials = 10
        ...
        # Reload Universal Weights
        model.load_state_dict(universal_weights)
        
        # First, test Zero-Shot performance BEFORE calibration
        model.eval()
        ...
        zeroshot_aurocs.append(zs_auc)
        print(f"  Zero-Shot AUROC (No Calibration): {zs_auc:.4f}")
        ...
        # Unfreeze EEG Encoder BatchNorm (Affine weights + running stats)
        ...
        # Calibrate for 50 epochs
        for epoch in range(CALIB_EPOCHS):
            # model.train() is CRITICAL here so BatchNorm1d updates running_mean and running_var on the new subject!
            model.train() 
            for b_e, b_a, b_b, b_y in calib_loader:
                ...
                identity = torch.eye(8, device=device)
                # Relaxed regularization from 0.01 to 0.001
        ...
        calibrated_aurocs.append(calib_auc)
        print(f"  Calibrated AUROC (1 min data):    {calib_auc:.4f}")
        
    print("\n=======================================================")
    print(f" FINAL RESULTS ACROSS {len(test_subjs)} UNSEEN TEST SUBJECTS:")
    print(f" Average Zero-Shot AUROC:  {np.mean(zeroshot_aurocs):.4f}")
    print(f" Average Calibrated AUROC: {np.mean(calibrated_aurocs):.4f}")
    print("=======================================================\n")
