...
def evaluate_filterbank_mtrf(W_dict, trials):
    y_preds = []
    y_labels = []
        ...
        for i in range(len(boundaries) - 1):
            ...
            if safe_end - safe_start >= SEQ_SAMPLES:
                for seq_start in range(safe_start, safe_end - SEQ_SAMPLES + 1, seq_hop):
                    ...
                    score_diff = score_L_total - score_R_total
                    label_L = 1.0 if current_spk == 'L' else 0.0
                    
                    y_preds.append(score_diff)
...
def main():
    ...
    print(f"\n=======================================================")
    print(f" PHASE 110: FILTER-BANK mTRF (Frequency Phenotypes)")
    print(f" Strict Mathematical Ensembling across Delta/Theta/Alpha/Beta")
    print(f"=======================================================\n", flush=True)
    ...
    for cache_file in filtered_files:
        ...
        print("  [Training] Fitting Independent mTRFs per frequency band...", flush=True)
        for band_idx, band_name in enumerate(BANDS.keys()):
            X_train, Y_train = extract_mtrf_matrices(raw_train_trials, band_idx)
            
            # Simple Ridge Sweep on the Train set using Leave-One-Out or fixed. 
            # We will use a fixed Lambda=100.0 to prevent overfitting and speed up execution
            # Since we are heavily regularizing through the Filter-Bank summation, an exact lambda sweep is less critical.
            W_band = fit_ridge(X_train, Y_train, lam=100.0)
            W_dict[band_name] = W_band
            print(f"    - {band_name} mTRF fitted (Lambda=100).", flush=True)
            
        print("  [Deployment] Ensembling Pearson R scores on Unseen Eval Set...", flush=True)
        eval_preds, eval_labels = evaluate_filterbank_mtrf(W_dict, raw_eval_trials)
        
        deployment_best_auc = 0
        if len(np.unique(eval_labels)) > 1:
            auc = roc_auc_score(eval_labels, eval_preds)
            # NO BLIND INVERSION. Pure raw physiological AUROC.
            deployment_best_auc = auc
            
        print(f"  [Deployment] Final Filter-Bank AUROC: {deployment_best_auc:.4f}")
        final_results[subj_name] = deployment_best_auc

    print("\n\n=======================================================")
    print(" PHASE 110 FILTER-BANK mTRF RESULTS")
    print("=======================================================")
